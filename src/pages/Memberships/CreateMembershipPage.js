...
const CreateMembershipPage = () => {
  ...
  const validateForm = () => {
    ...
    if (AllBenifits.length == 0) errors.benefits = 'Provide at least one benefit.';
    ...
    if (formData.type === 'senior' && (!formData.age || Number(formData.age) <= 0)) {
  ...
  const handleSubmit = async () => {
    ...
    // alert("colling")
    try {
      const response = await axios.post('http://localhost:8000/api/memberships', {
        ...formData,
